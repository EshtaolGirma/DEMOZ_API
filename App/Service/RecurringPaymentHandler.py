...
def GetListOfBillPlan(user):
    ...
    for bill in bills:
        x = {'title': bill.bill_title,
             ...
             'Deposit Date': [
                 bill.next_payment_date.day,
                 bill.next_payment_date.month,
                 bill.next_payment_date.year,
             ],
             'id': bill.id
             }
        li.append(x)
...
def GetBillPlanDetail(user, bill_id):
    ...
    result = {'Bill': [{
        ...
        'Bill Started On': [
            bill.starting_date.day,
            bill.starting_date.month,
            bill.starting_date.year
        ],
...
def UpdateBillPlanDetail(user, bill_id, request):
    try:
        ...
        if request.json['bill_description'] != '' and current_info.description != request.json['bill_description']:
            current_info.description = request.json['bill_description']
